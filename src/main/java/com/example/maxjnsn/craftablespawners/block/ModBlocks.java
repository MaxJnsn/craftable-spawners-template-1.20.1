package com.example.maxjnsn.craftablespawners.block;

import com.example.maxjnsn.craftablespawners.CraftableSpawners;
import com.example.maxjnsn.craftablespawners.block.custom.EmptySpawner;
import net.fabricmc.fabric.api.item.v1.FabricItemSettings;
import net.fabricmc.fabric.api.object.builder.v1.block.FabricBlockSettings;
import net.minecraft.block.Block;
import net.minecraft.block.Blocks;
import net.minecraft.item.BlockItem;
import net.minecraft.item.Item;
import net.minecraft.registry.Registries;
import net.minecraft.registry.Registry;
import net.minecraft.util.Identifier;

public class ModBlocks {
    public static final Block EMPTY_SPAWNER = registerBlock("empty_spawner",
            new EmptySpawner(FabricBlockSettings.copyOf(Blocks.SPAWNER)));



    private static Block registerBlock(String name, Block block) {
        registerBlockItem(name, block);
        return Registry.register(Registries.BLOCK, new Identifier(CraftableSpawners.MOD_ID, name), block);
    }

    private static Item registerBlockItem(String name, Block block) {
        return Registry.register(Registries.ITEM,
                new Identifier(CraftableSpawners.MOD_ID, name),
                new BlockItem(block, new FabricItemSettings()));
    }

    public static void registerModBlocks() {
        CraftableSpawners.LOGGER.info("Registering block items for " + CraftableSpawners.MOD_ID);
    }
}
